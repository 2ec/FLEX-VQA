...
def generate_explanations_for_one_image(image_name):
    ...
    # max length of sentence, or X_sentence.shape[-1]
    ...
    # -------- LOAD MODEL --------
    model_path = os.path.join(params.MODEL_SAVE_FOLDER,
                              opt.model_version, 'flex')
    ...
    sentence, indices = flex_model.get_explanation(
        feat, id_to_word, word_to_id)
    ...
    class_name = " ".join(i.capitalize()
                          for i in image_name.lower().split('/')[-1].split("_")[:-2])
...
def generate_explanations():
    ...
    target = open(os.path.join(params.MODEL_SAVE_FOLDER, opt.model_version,
                  'explanations_' + opt.model_version+".txt"), 'w')
    target.write('\n'.join(explanations))
    target.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', default='generate_exp',
                        help='Which method you want to execute')
    parser.add_argument('--model_version', default='flex_v1',
                        help='Name of the trained model')
    parser.add_argument('--image_name', default='Grasshopper_Sparrow_0078_116052.jpg',
                        help='The test image for which you want to generate the explanation')
