...
class FLEX(object):
    ...
    def __init__(self, params):
        ...
        # -------- Placeholders --------
        self.img_feats = tf.compat.v1.placeholder(tf.float32, [None, self.img_feature_size])
        self.batch_size = tf.compat.v1.placeholder(tf.int32, [])
        self.seq_lengths = tf.compat.v1.placeholder(tf.int32, [None])
        self.rnn_inputs = tf.compat.v1.placeholder(tf.int32, [None, self.max_length])
        self.rnn_outputs_input = tf.compat.v1.placeholder(tf.int32, [None, self.max_length])
        self.c_state1 = tf.compat.v1.placeholder(tf.float32, [None, self.num_hidden_lstm])
        self.h_state1 = tf.compat.v1.placeholder(tf.float32, [None, self.num_hidden_lstm])
        self.c_state2 = tf.compat.v1.placeholder(tf.float32, [None, self.num_hidden_lstm])
        self.h_state2 = tf.compat.v1.placeholder(tf.float32, [None, self.num_hidden_lstm])
        self.learning_rate_input = tf.compat.v1.placeholder(tf.float32, [])
        self.keep_prob = tf.compat.v1.placeholder(tf.float32, [])
        self.scores_input = tf.compat.v1.placeholder(tf.float32, [None, self.max_length, self.vocab_size])

        self.initial_state2 = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(self.c_state2, self.h_state2)
        self.initial_state1 = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(self.c_state1, self.h_state1)
        ...
        self.w_embedding = tf.Variable(tf.random.uniform([self.vocab_size, self.embedding_size], -1.0, 1.0))
        self.embed_word = tf.nn.embedding_lookup(params=self.w_embedding, ids=self.rnn_inputs)
        ...
        self.img_embedding_bias = tf.Variable(tf.compat.v1.constant_initializer(0.0)((self.img_embed_size,)), name="img_embed_bias")
        ...
        with tf.compat.v1.variable_scope('rnn1'):
            if self.dropout:
                self.lstm_cell1 = tf.compat.v1.nn.rnn_cell.DropoutWrapper(tf.compat.v1.nn.rnn_cell.LSTMCell(self.num_hidden_lstm, state_is_tuple=True), output_keep_prob=self.keep_prob)
            else:
                self.lstm_cell1 = tf.compat.v1.nn.rnn_cell.LSTMCell(self.num_hidden_lstm, state_is_tuple=True)
            self.zero_initial_state1 = self.lstm_cell1.zero_state(self.batch_size, tf.float32)
            self.lstm_outputs1, self.lstm_states1 = tf.compat.v1.nn.dynamic_rnn(self.lstm_cell1, self.embed_word, dtype=tf.float32, sequence_length=self.seq_lengths,
        ...
        self.lstm2_embed_b = tf.Variable(tf.compat.v1.constant_initializer(0.0)((self.num_hidden_lstm,)), name="lstm_2_embed_bias")
        ...
        self.lstm_outputs1 = tf.transpose(a=self.lstm_outputs1, perm=[1, 0, 2])
        self.lstm_inputs2 = tf.map_fn(self.hidden_projection, self.lstm_outputs1)
        self.lstm_inputs2 = tf.transpose(a=self.lstm_inputs2, perm=[1, 0, 2])

        # -------- LSTM 2 --------
        with tf.compat.v1.variable_scope('rnn2'):
            if self.dropout:
                self.lstm_cell2 = tf.compat.v1.nn.rnn_cell.DropoutWrapper(tf.compat.v1.nn.rnn_cell.LSTMCell(self.num_hidden_lstm, state_is_tuple=True), output_keep_prob=self.keep_prob)
            else:
                self.lstm_cell2 = tf.compat.v1.nn.rnn_cell.LSTMCell(self.num_hidden_lstm, state_is_tuple=True)
            self.zero_initial_state2 = self.lstm_cell2.zero_state(self.batch_size, tf.float32)
            self.lstm_outputs2, self.lstm_states2 = tf.compat.v1.nn.dynamic_rnn(self.lstm_cell2, self.lstm_inputs2, dtype=tf.float32, sequence_length=self.seq_lengths,
        ...
        self.output_b = tf.Variable(tf.compat.v1.constant_initializer(0.0)((self.vocab_size,)), name="output_bias")

        self.logit_projection = lambda x: tf.matmul(x, self.output_w) + self.output_b

        # -------- Project output of LSTM to output layer --------
        self.lstm_outputs2 = tf.transpose(a=self.lstm_outputs2, perm=[1, 0, 2])
        self.final_logits = tf.map_fn(self.logit_projection, self.lstm_outputs2)
        self.final_logits = tf.transpose(a=self.final_logits, perm=[1, 0, 2])
        self.sentence_word_probs = tf.nn.softmax(self.final_logits, axis=-1)

        # -------- Calculate relevance score --------
        score = tf.multiply(self.sentence_word_probs, self.scores_input)
        score = tf.reduce_max(input_tensor=score, axis=2)

        # -------- Calculate cross entropy loss --------
        self.cross_entropy_loss = self.calculate_cross_entropy(score)

        # -------- Calculate final loss --------
        self.final_loss = tf.reduce_mean(input_tensor=self.cross_entropy_loss)

        # -------- Optimizer --------
        self.optimize_step = tf.compat.v1.train.AdamOptimizer(self.learning_rate_input).minimize(self.final_loss)
    ...
    def calculate_cross_entropy(self, relevance_score):  # A function from sbarratt's interpnet code
        ...
        range_row = tf.transpose(a=tf.tile(range_row, [1, self.batch_size]))
        mask_int = tf.less(range_row, lengths_transposed)
        mask = tf.compat.v1.where(mask_int, ones, zeros)
        cross_entropy = self.lstm_outputs * tf.math.log(self.sentence_word_probs + 1e-8)
        cross_entropy = -tf.reduce_sum(input_tensor=cross_entropy, axis=2)
        cross_entropy = cross_entropy * tf.cast(mask, tf.float32)
        cross_entropy = tf.reduce_sum(input_tensor=tf.subtract(cross_entropy, tf.multiply(relevance_score, self.lambda_value)), axis=1)
